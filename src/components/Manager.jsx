...
const Manager = () => {
	...
	const checkWebsiteValidity = async function (url) {
		...
		try {
			...
			if (response.status >= 200 && response.status < 400) {
				return { valid: true, reason: "URL is reachable" };
			} else {
				return {
					valid: false,
	...
	const savePassword = async () => {
		const urlValid = await checkWebsiteValidity(form.site);
		if (
			urlValid.valid &&
			...
			<div className="">
				<div className="text-center mb-10">
					<h2 className="text-4xl font-bold text-gray-800">
						<span className="text-green-500">Pass</span>OP - Your own
						Password Manager
					</h2>
				</div>
				{/*Form section */}
